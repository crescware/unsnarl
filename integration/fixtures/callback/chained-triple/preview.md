# integration/fixtures/callback/chained-triple/input.ts

## Input

```ts
const arr = [1, 2, 3];
const xs = arr.map((v) => v + 1).filter((v) => v > 0).map((v) => v * 2);
```
...
flowchart RL
  ...
  subgraph wrap_call_proxy_34[" "]
    ...
    subgraph call_proxy_34["arr.map().filter().map()<br/>L2"]
      direction RL
      subgraph call_proxy_34_76["arr.map().filter()<br/>L2"]
        direction RL
      ...
      subgraph s_scope_3["arr.map().filter().map(args[0])<br/>L2"]
        direction RL
        n_scope_3_v_82["v<br/>L2"]
        subgraph s_return_scope_3_88_93["return L2"]
          direction RL
          ret_use_ref_5["v<br/>L2"]
        end
      end
    end
  end
