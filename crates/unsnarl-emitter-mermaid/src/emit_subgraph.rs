...
//! is known. Two subgraph kinds carry an owner: a named `Function`
//! (owner = the FunctionName node) and a result-bound `CallProxy`
//! (owner = the result-variable node, e.g. `xs` in
//! `const xs = arr.map(cb)`). Both express the owner ↔ subgraph
//! relationship by containment in a shared `wrap_<id>` box.
...
pub fn emit_subgraph(state: &mut RenderState<'_>, sg: &VisualSubgraph, indent: &str, depth: u32) {
    // `owner_node_id` is `Some` only for a named Function or a
    // result-bound CallProxy; both get the same wrapper treatment.
    if let Some(owner_id) = sg.owner_node_id() {
        if let Some(&owner_node) = state.node_map.get(owner_id) {
            ...
            // to the parent scope (it names the function, or binds the
            // call result, from the outside), so it must NOT live
            // inside the body subgraph. The wrapper exists purely to
            // keep these two siblings adjacent in the rendered diagram.
