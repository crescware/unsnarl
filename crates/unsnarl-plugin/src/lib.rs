...
//!
//! The `UnsnarlPlugin` shape is exposed as a trait, with a small
//! registry that the `unsnarl` crate uses to map CLI plugin names
//! to concrete plugin instances. The trait + registry are pure
//! IR-side machinery; they intentionally depend only on
