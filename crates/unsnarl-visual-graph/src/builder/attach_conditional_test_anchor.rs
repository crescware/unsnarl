...
/// it gates. A consequent that collapsed past the depth threshold
/// builds no subgraph; with nowhere to host the anchor it is dropped
...
pub fn attach_conditional_test_anchor(
    ...
    if state
        .conditional_test_anchor_by_offset
        .contains_key(&offset.0)
    {
        return;
    }
