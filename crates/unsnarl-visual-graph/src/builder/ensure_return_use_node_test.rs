...
use crate::builder::builder_fixtures::{
    ...
    jsx_container, reference_id, return_completion, scope_id, span_offset_line, variable_id,
};
...
fn returns_none_when_no_host_subgraph_exists() {
    ...
    let id = ensure_return_use_node(&mut arena, &mut state, &ctx, Some("fnVar"), None, &r);
    assert_eq!(id, None);
}
...
fn uses_identifier_name_when_variable_is_not_resolved() {
    ...
    ensure_return_use_node(&mut arena, &mut state, &ctx, Some("fnVar"), None, &r);
    let ret_idx = first_subgraph_child(&arena, host);
    let node_idx = first_node_child(&arena, ret_idx);
...
fn groups_refs_with_same_return_completion_offsets_into_one_subgraph() {
    ...
    ensure_return_use_node(&mut arena, &mut state, &ctx, Some("fnVar"), None, &r1);
    ensure_return_use_node(&mut arena, &mut state, &ctx, Some("fnVar"), None, &r2);
    assert_eq!(arena.subgraph(host).children.len(), 1);
    let ret_idx = first_subgraph_child(&arena, host);
...
/// walk (`subgraph_by_scope`), and the wrapping Return subgraph is
...
fn keys_and_names_subgraph_by_scope_id_when_no_owner_var() {
    ...
    let r = ret_ref("r1", 12, 34, 1, 2);
    let id = ensure_return_use_node(&mut arena, &mut state, &ctx, None, Some("scope"), &r);
    assert_eq!(id.as_deref(), Some("ret_use_r1"));
    assert!(state.return_subgraphs_by_fn.contains_key("scope"));
    let ret_idx = first_subgraph_child(&arena, host);
    let VisualSubgraph::Owned(sg) = arena.subgraph(ret_idx).descriptor.clone() else {
        panic!("expected owned");
    };
    assert_eq!(sg.id, "s_return_scope_12_34");
}
...
fn returns_none_without_owner_var_and_without_host() {
    ...
    let mut state = BuildState::new();
    let r = ret_ref("r1", 0, 10, 1, 1);
    let id = ensure_return_use_node(&mut arena, &mut state, &ctx, None, None, &r);
    assert_eq!(id, None);
}
