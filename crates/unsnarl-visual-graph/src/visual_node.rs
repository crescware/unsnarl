...
impl VisualNode {
    ...
    ///
    /// Mirrors the TS code paths that branch on `n.kind`. The two
    /// underlying shapes (binding / synthetic) carry their own
    /// subset enums so the on-disk JSON field order can differ; this
    /// projects them back onto a single flat [`NodeKind`] so
    ...
    /// `importedName` from the original TS shape, present only on
    /// `NamedImportBinding`.
    ...
    /// `declarationKind` from the original TS shape, present only on
    /// `WriteReference`. The TS form keeps `null` when the
    /// underlying variable is not a declaration (e.g. an
    /// implicit-global reassignment); that maps to `Some(None)` at
    /// the JSON level but the caller only cares whether a concrete
    /// kind is present, so the flattened `Option<...>` form is
    /// returned here.
    ...
    /// `initIsFunction` from the original TS shape, present only on
    /// `VarBinding` / `ConstBinding` / `LetBinding`. Returns `false`
    /// for every other kind so callers can branch without a separate
    /// presence check (the TS code paths fall back to `false` the
    /// same way).
