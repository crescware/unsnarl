...
//!
//! The constants double as the on-disk JSON discriminator strings
//! (`"node"` / `"subgraph"`); each side is its own unit enum so it
//! sits in a struct's `type` field as a const string.
