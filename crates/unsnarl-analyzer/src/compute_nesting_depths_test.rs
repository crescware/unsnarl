...
fn function_body_increments_function_counter() {
    ...
    let fn_idx = code.find("function").unwrap() as u32;
...
fn if_body_increments_if_independently_of_function() {
    ...
    let inner_idx = code.find("{ let y").unwrap() as u32;
...
fn for_init_binding_lives_at_parent_depth() {
    ...
    let for_idx = code.find("for ").unwrap() as u32;
    let d_for = at(code, for_idx);
    assert_eq!(d_for.r#for, NestingDepth(0));
    let body_idx = code.find("{ i;").unwrap() as u32;
...
fn nested_if_inside_if_increments_if_to_two() {
    ...
    let inner_idx = code.find("{ 1;").unwrap() as u32;
    let d = at(code, inner_idx);
...
fn empty_if_body_still_counts() {
    let code = "if (a) { foo(); }";
    let inner_idx = code.find("{ foo").unwrap() as u32;
    let d = at(code, inner_idx);
    assert_eq!(d.r#if, NestingDepth(1));
}
...
fn arrow_function_increments_function_counter() {
    let code = "const f = () => { 1; };";
    let arrow_idx = code.find("() =>").unwrap() as u32;
...
fn switch_statement_increments_switch_counter() {
    ...
    let switch_idx = code.find("switch").unwrap() as u32;
...
fn while_body_increments_while_counter() {
    ...
    let body_idx = code.find("{ foo").unwrap() as u32;
...
fn try_catch_finally_increments_try_catch_finally() {
    ...
    let try_block_idx = code.find("{ a;").unwrap() as u32;
    let catch_body_idx = code.find("{ b;").unwrap() as u32;
    let finally_body_idx = code.find("{ c;").unwrap() as u32;
