...
fn returns_none_for_standalone_if() {
    ...
    let path = vec![
        ...
        PathEntry::new(parent.clone(), Some("body")),
    ];
    assert_eq!(
...
fn walks_back_through_multiple_chained_alternates() {
    ...
    let path = vec![
        PathEntry::new(ast_node(AstType::Program, 0), None),
        PathEntry::new(outermost, Some("body")),
        PathEntry::new(middle, Some("alternate")),
        PathEntry::new(innermost.clone(), Some("alternate")),
    ];
    assert_eq!(
...
fn does_not_walk_past_a_non_if_statement_ancestor() {
    ...
    let path = vec![
        PathEntry::new(ast_node(AstType::Program, 0), None),
        PathEntry::new(outer, Some("body")),
        PathEntry::new(inner_block, Some("alternate")),
        PathEntry::new(inner.clone(), Some("body")),
    ];
    assert_eq!(
        if_chain_root_offset(Some(&inner), Some("consequent"), &path),
        None
...
fn does_not_walk_when_current_if_sits_in_consequent_slot_of_outer() {
    ...
    let path = vec![
        PathEntry::new(ast_node(AstType::Program, 0), None),
        PathEntry::new(outer, Some("body")),
        PathEntry::new(cons_block, Some("consequent")),
        PathEntry::new(inner.clone(), Some("body")),
    ];
    assert_eq!(
        if_chain_root_offset(Some(&inner), Some("consequent"), &path),
