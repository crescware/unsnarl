...
import { computeCategoryDepths } from "./compute-category-depths.js";
...
function depthsAt(
  ...
  const map = computeCategoryDepths(ast);
  return map.get(offset);
}

describe("computeCategoryDepths", () => {
  test("function body increments function counter", () => {
    ...
    expect(at?.[CATEGORY.Function]).toBe(1);
  });
  ...
  test("for-init binding lives at parent depth (() outside {})", () => {
    ...
    expect(forDepths?.[CATEGORY.For]).toBe(0);

    const bodyIdx = code.indexOf("{ i;");
    const bodyDepths = depthsAt(code, bodyIdx);
    expect(bodyDepths?.[CATEGORY.For]).toBe(1);
  });
  ...
  test("nested if inside if increments if to 2", () => {
    ...
    const at = depthsAt(code, innerIdx);
    expect(at?.[CATEGORY.If]).toBe(2);
  });

  test("each category counts independently: if inside for stays at if=1, for=1", () => {
    const code = "for (;;) { if (b) { 1; } }";
    const innerIdx = code.indexOf("{ 1;");
    const at = depthsAt(code, innerIdx);
    expect(at?.[CATEGORY.For]).toBe(1);
    expect(at?.[CATEGORY.If]).toBe(1);
  });
